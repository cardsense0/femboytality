local Fatality;
if isfile and isfile("femboytality/UI/source.lua") then
	Fatality = loadstring(readfile("femboytality/UI/source.lua"))();
else
	-- Note: You are running the remote github version, which does NOT have our recent changes!
	-- If you're copying this into your executor, copy all of source.lua and paste it above this code instead,
	-- or put source.lua in your executor's workspace/femboytality/UI/source.lua folder!
	Fatality = loadstring(game:HttpGet("https://raw.githubusercontent.com/cardsense0/femboytality/refs/heads/main/menu.lua"))();
end
...
do
	...
	local GeneralCat = AimbotSub:AddSidebarCategory({ Name = "General" })
	local AccuracyCat = AimbotSub:AddSidebarCategory({ Name = "Accuracy" })
	
	local Weapon = GeneralCat:AddSection({ Position = 'left', Name = "WEAPON" });
	local Extra = GeneralCat:AddSection({ Position = 'right', Name = "EXTRA" });
	local General = GeneralCat:AddSection({ Position = 'left', Name = "GENERAL" });
...
do
	...
	local MainCat = AimbotSub:AddSidebarCategory({ Name = "Main" })
	
	local Aim = MainCat:AddSection({ Position = 'left', Name = "AIM" });
	local Rcs = MainCat:AddSection({ Position = 'right', Name = "RCS" });
	local Backtrack = MainCat:AddSection({ Position = 'left', Name = "BACKTRACK" });
	local General = MainCat:AddSection({ Position = 'right', Name = "GENERAL" });
...
do
	...
	local EnemiesCat = EspSub:AddSidebarCategory({ Name = "Enemies" })
	
	local Misc = EnemiesCat:AddSection({ Name = "MISC", Position = 'left' })
	local Model = EnemiesCat:AddSection({ Name = "MODEL", Position = 'right' })
